...
int inter_guild_tosql(struct guild *g,int flag)
{
	...
	*t_info = '\0';
	...
	if(((flag & GS_BASIC_MASK) && !new_guild) || ((flag & (GS_BASIC_MASK & ~GS_BASIC)) && new_guild)) {
		...
		if(SQL_ERROR == SQL->Query(sql_handle, "%s", StringBuf_Value(&buf)))
			Sql_ShowDebug(sql_handle);
...
int inter_guild_ReadEXP(void)
{
	...
	if(SQL_ERROR == SQL->Query(sql_handle, "SELECT * FROM `%s`.`%s`", tmp_db_name, "exp_guild_db")) {
		Sql_ShowDebug(sql_handle);
		return -1;
	}

	while(SQL_SUCCESS == SQL->NextRow(sql_handle)) {
		SQL->GetData(sql_handle, 0, &row, NULL);
