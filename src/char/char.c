...
int mmo_char_tosql(int char_id, struct mmo_charstatus *p)
{
	...
	if(memcmp(p->memo_point, cp->memo_point, sizeof(p->memo_point))) {
		...
		if(count) {
			if(SQL_ERROR == SQL->QueryStr(sql_handle, StringBuf_Value(&buf))) {
				Sql_ShowDebug(sql_handle);
				errors++;
			}
		}
...
int memitemdata_to_sql(const struct item items[], int max, int id, int tableswitch)
{
	...
	if(found && SQL_ERROR == SQL->QueryStr(sql_handle, StringBuf_Value(&buf))) {
		Sql_ShowDebug(sql_handle);
		errors++;
	}
...
int make_new_char_sql(struct char_session_data *sd, char *name_, int str, int agi, int vit, int int_, int dex, int luk, int slot, int hair_color, int hair_style)
{
	...
	{ //Adicionado suporte para múltiplos itens [brAthena]
		...
		while(start_items[i].id != 0 && i < MAX_START_ITEMS) {
			...
			for(k = 0; k < qt; k += j)
				if(SQL_ERROR == SQL->Query(sql_handle, "INSERT INTO `%s` (`char_id`,`nameid`, `amount`, `identify`) VALUES ('%d', '%d', '%d', '%d')", inventory_db, char_id, id, j, 1))
					Sql_ShowDebug(sql_handle);
			i++;
		}
	}
...
int delete_char_sql(int char_id)
{
	...
	SQL->GetData(sql_handle, 9, &data, NULL); elemental_id = atoi(data);
					...
					return 0;
				{
					...
					if(acc > 0) {
						...
						for(i = 0; i < num; ++i) {
							...
							|| SQL_ERROR == SQL->Query(sql_handle, "UPDATE `%s` SET `equip`='0' WHERE `char_id`='%d' AND `equip` != '0'", inventory_db, char_id[i]))
								Sql_ShowDebug(sql_handle);
...
//					result = 2; // 2-gm level too low
							...
							switch(type) {
									...
									/* handled by char server, so no redirection */
								{
									...
									if(SQL_SUCCESS != SQL->StmtPrepare(stmt, "UPDATE `%s` SET `unban_time` = ? WHERE `char_id` = ? LIMIT 1",char_db)
